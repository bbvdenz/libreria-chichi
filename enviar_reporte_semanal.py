"""
Reporte semanal de ventas de Libreria Chichi.
Calcula lo vendido en los ultimos 7 dias y lo envia por correo a la duenia,
adjuntando un Excel (.xlsx) con el historico completo como respaldo.

Uso (con el entorno activado, dentro de websistemasinfo):
    python enviar_reporte_semanal.py

Para que se envie solo cada semana, programalo con el Programador de tareas de
Windows: una tarea semanal que ejecute:
    C:\\...\\venv\\Scripts\\python.exe  C:\\...\\websistemasinfo\\enviar_reporte_semanal.py

Requisitos: pip install openpyxl
Para que el correo salga de verdad, configura SMTP en settings.py.
"""
import os
import io
import django
...
# ── CONFIGURA AQUI el correo de la duenia ──
DESTINATARIO = 'duenia@libreriachichi.cl'
...
def reporte_semanal():
    ...
    # Detalles de la semana (para el cuerpo del correo)
    det_semana = list(
        DetallePedido.objects.select_related('id_pedido', 'id_producto')
        .filter(id_pedido__estado_pedido__in=ESTADOS_VENTA,
                id_pedido__fecha_pedido__gte=desde, id_pedido__fecha_pedido__lte=hoy)
    )
    resumen_sem, prods_sem, _, _ = _agrupar(det_semana)

    # Todos los detalles (para el Excel historico)
    det_todos = list(
        DetallePedido.objects.select_related('id_pedido', 'id_producto')
        .filter(id_pedido__estado_pedido__in=ESTADOS_VENTA)
    )
    ...
    rango = f"{desde.strftime('%d/%m/%Y')} al {hoy.strftime('%d/%m/%Y')}"
    asunto = f"Reporte de ventas Libreria Chichi - {rango}"

    top = sorted(prods_sem.values(), key=lambda x: -x['ingresos'])
    lineas = [
        f"Reporte semanal de ventas - {rango}", "",
        f"Ventas (pedidos): {resumen_sem['n_ventas']}",
        f"Productos vendidos: {resumen_sem['unidades']}",
        f"Dinero generado: ${clp(resumen_sem['ingresos'])}", "",
        "Productos mas vendidos:",
    ]
    lineas += [f"  - {p['unidades']}x {p['nombre']} ({p['sku']}) -> ${clp(p['ingresos'])}" for p in top] or ["  (sin ventas)"]
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;color:#222;">
      ...
      <div style="display:flex;gap:10px;margin:18px 0;">
        <div style="flex:1;background:#faf5ff;border:1px solid #e9d5ff;border-radius:10px;padding:14px;text-align:center;">
          ...
          <div style="font-size:1.6rem;font-weight:900;">{resumen_sem['n_ventas']}</div></div>
        <div style="flex:1;background:#faf5ff;border:1px solid #e9d5ff;border-radius:10px;padding:14px;text-align:center;">
          <div style="font-size:0.75rem;color:#7c3aed;font-weight:700;">PRODUCTOS</div>
          <div style="font-size:1.6rem;font-weight:900;">{resumen_sem['unidades']}</div></div>
        <div style="flex:1;background:#fdf2f8;border:1px solid #fbcfe8;border-radius:10px;padding:14px;text-align:center;">
          ...
          <div style="font-size:1.6rem;font-weight:900;color:#e8007d;">${clp(resumen_sem['ingresos'])}</div></div>
      </div>
    ...
    excel_bytes = construir_excel(prods_sem, por_dia, por_sem, resumen_sem, rango)
    nombre_excel = f"ventas_libreria_chichi_{hoy.strftime('%Y-%m-%d')}.xlsx"
    ...
    msg.attach(nombre_excel, excel_bytes,
               'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    msg.send()

    print(f"Reporte enviado a {DESTINATARIO} ({rango}) con Excel adjunto: {nombre_excel}")
    print(f"  Ventas: {resumen_sem['n_ventas']} | Productos: {resumen_sem['unidades']} | Dinero: ${clp(resumen_sem['ingresos'])}")
